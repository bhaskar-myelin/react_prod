// import { API_CONSTANTS, CONSTANTS } from "../../utils/constants";
// import {
//   convertNormalOptions,
//   queryParamsToString,
//   sheetConfigBuilder,
// } from "../../utils/handlers";
...
// import { ACTION_LIST, OPERATIONS_LIST } from "../../utils/dataset";
// import { BaseAnalysisFn } from "../../utils/classes/Base";
// import moment from "moment";
// import Axios from "axios";

// export const fetchTableOptions =
//   (options, callback, showLoader = true) =>
...
//       let sheetId =
//         options.sheetData &&
//         options.sheetData.rows &&
//         options.sheetData.rows.length
//           ? options.sheetData.rows[0].id
//           : "";
//       if (options.sheet) {
//         sheetId = options.sheet;
//       }
//       let FETCHED_DATA = await fetchScripData(
//         { ...options, sheetId },
//         store(),
//         dispatch
//       );
...
//       let sheetDataValues =
//         options.sheetData && options.sheetData.rows
//           ? options.sheet
//             ? _.find(options.sheetData.rows, { id: options.sheet })
//             : options.sheetData.rows[0]
//           : {};
//       let analysedData = BaseAnalysisFn({
//         sheetConfig: sheetDataValues || {},
//         sheetData: options.sheetData || [],
//         ScripRows:
//           FETCHED_DATA.scripList && FETCHED_DATA.scripList.adjusted
//             ? FETCHED_DATA.scripList.adjusted
//             : [],
//         initialIndex:
//           FETCHED_DATA.initialIndex || FETCHED_DATA.nearestIndex
//             ? FETCHED_DATA.initialIndex || FETCHED_DATA.nearestIndex
//             : 0,
//         OHLCParam: options.baseType || 4,
//         bonusList: FETCHED_DATA.bonusList || [],
//         LatestScripRows:
//           FETCHED_DATA.scripList && FETCHED_DATA.scripList.latest
//             ? FETCHED_DATA.scripList.latest
//             : [],
//         props:
//           store().records && store().records.hypotheticalProps
//             ? store().records.hypotheticalProps
//             : {},
//         historicalConfig:
//           store().records && store().records.historicalConfig
//             ? store().records.historicalConfig
//             : {},
//       });
...
//           dateMap: FETCHED_DATA.scripList.indexing
//             ? FETCHED_DATA.scripList.indexing.reverse()
//             : [],
//           bonusList: FETCHED_DATA.bonusList || [],
...
//       if (
//         store().records &&
//         store().records[options.routeType] &&
//         store().records[options.routeType][key]
//       ) {
...
//       let apiInstance = new ApiBuilder(
//         `${API_CONSTANTS[routeType][operationType]}${
...
//   (options, params, callback) => async (dispatch, store) => {
//     let key = convertNormalOptions(options);
//     try {
//       dispatch({
//         type: LOADING,
...
//       if (
//         params.operationType === ACTION_LIST.EDIT &&
//         params.freshOptions.sheet
//       ) {
//         p["id"] = params.freshOptions.sheet;
//       }
//       let sheetPayload = sheetConfigBuilder({ ...p });
//       let apiInstance = new ApiBuilder(
//         `${API_CONSTANTS[params.routeType][CONSTANTS.operation.EDIT]}`,
//         {
//           headers: {
//             authorization: store().auth.token,
//           },
...
//         let findRowIndex = _.findIndex(reduxPayload.rows, {
//           id: sheetResponse.data.id,
//         });
//         if (findRowIndex > -1) {
...
//       _.omit(options, ["data", "mediaType", "routeType", "operationType"])
//     );
//     let explicitType = "";
//     if (responsePayload.data) {
//       let resourceKeys = Object.keys(store().records["scripDetails"]);
//       let resourceBaseKey = _.find(resourceKeys, (e) =>
//         e.includes(key) ? e : ""
//       );
//       let reduxPayload = {};
...
//       }
//       if (operationType === CONSTANTS.operation.EDIT) {
//         let findRowIndex = _.findIndex(reduxPayload.media, {
...
//     queryParams = queryParamsToString(queryParams);
//     let key = convertNormalOptions(
//       _.omit(options, ["sheetId", "operationType", "routeType", "documentId"])
//     );
...
//       let reduxPayload = {};
//       if (pload.key === "sheets") {
//         resourceBaseKey = _.find(resourceKey, (e) =>
//           e.startsWith(key) ? e : ""
//         );
//         if (resourceBaseKey) {
//           reduxPayload = store().records[pload.key][resourceBaseKey];
//         }
//         reduxPayload.rows = _.map(
//           _.pickBy(reduxPayload.rows, (e) => e.id !== pload.value)
//         );
//         reduxPayload.count = reduxPayload.count - 1;
...
//         };
//       } else {
//         resourceBaseKey = _.filter(resourceKey, (e) =>
//           e.includes(key) ? e : ""
//         );
...
//       operationType,
//       routeType,
//       payload: { data: remarks, scripDataId },
//     } = options;
...
//     let sheetId =
//       data && data.freshOptions && data.freshOptions.sheet
//         ? data.freshOptions.sheet
//         : data.sheetData.rows.length
//         ? data.sheetData.rows[0].id
//         : "";
//     let sheetConfig = {};
//     if (sheetId) {
...
// export const BonusOperationHandler =
//   (options, callback) => async (dispatch, store) => {
//     try {
//       dispatch({
//         type: LOADING,
//       });
...
//         announcementDate:
//           payload.announcementDate || moment().format("YYYY-MM-DD"),
//       };
//       let errLoad = {};
...
//           };
//           if (
//             isNaN(requestPayload.config.value1) ||
//             isNaN(requestPayload.config.value2)
//           ) {
//             errLoad = {
//               hasError: true,
...
//       // API Instance
//       let apiInstance = new ApiBuilder(
//         `${API_CONSTANTS.scripDetails.addBonus}`,
//         {
//           headers: {
//             authorization: store().auth.token,
//           },
...
// export const AddScripAnalytics =
//   (options, callback) => async (dispatch, store) => {
//     try {
//       dispatch({
//         type: LOADING,
...
//         operationType,
//         routeType,
//         payload: { scripGroup: group, scripId, scripIndustry: industry },
//       } = options;
//       let payload = {
//         scripId,
//         industry,
...
// export const RemoveAnalyticsScrip =
//   (options, callback) => async (dispatch, store) => {
//     try {
//       let {
//         operationType,
...
//         bool = false;
//         url = `${API_CONSTANTS[routeType][operationType]}${
//           queryParams ? `?${queryParams}` : ""
//         }`;
//       }
...
// export const ShareDocumentOverEmail =
//   (options, callback) => async (dispatch, store) => {
//     try {
//       dispatch({
//         type: LOADING,
...
//         };
//         if (options.query) {
//           if (options.query.industry) {
//             payload["industry"] = options.query.industry;
//           }
//           if (options.query.group) {
//             payload["group"] = options.query.group;
//           }
//         }
...
//         formData.append(
//           "data",
//           JSON.stringify({ scripId: options.match.scripId })
//         );
//       }
...
//         error.data &&
//         error.data.error &&
//         error.data.error.timestamp
//       ) {
//         t = Object.values(error.data.error.timestamp)
//           .map((e) => {
...
//           ? ""
//           : !t
//           ? `Data invalid in current file`
//           : `Data invalid for following timestamp: ${t}`,
//       });
//     }
//   };

// export const FetchReportDetails =
//   (options, callback) => async (dispatch, store) => {
//     try {
...
// const fetchScripData = (params, store, dispatch) =>
...
//     let url =
//       "stock/history?scripId=" + params.scripId + (queryP ? "&" + queryP : "");
...
// export const SetHypotheticalProps =
//   (data, callback) => async (dispatch, store) => {
//     try {
//       if (_.isEmpty(data)) {
//         dispatch({
//           type: MODIFY_RECORDS,
...
// export const AddEditStockHandler =
//   (options, callback) => async (dispatch, store) => {
//     try {
//       dispatch({
//         type: LOADING,
...
//         formData.append(
//           "file",
//           options.file && options.file.binary ? options.file.binary : ""
//         );
...
//           hasError: false,
//           message:
//             (response.data ? response.data.response : 0) ||
...
//         hasError: true,
//         message: bool
//           ? `Data Not present for following timestamp: ${Object.values(
//               error.data.error.timestamp
//             )
...
// export const DeleteStockHandlerFn = (options) => async (dispatch, store) =>
//   new Promise(async (resolve, reject) => {
//     dispatch({
//       type: LOADING,
//     });
...
// export const DefaultExportTable = (options) => async (dispatch, store) =>
//   new Promise(async (resolve, reject) => {
//     dispatch({
//       type: LOADING,
//     });
...
//           link.setAttribute(
//             "download",
//             `${options.type}-${new Date().toLocaleDateString()}.xlsx`
//           );
